...
void printlog(struct session *c)
{
	char log[1000];
						...
						sprintf(log,"%s Protocol: %d IPVersion:%d Source Address:%s:%d --> Destination Address:%s:%d Send:%ldKb Recieved:%ldKb\n",ctime(&t),c->protocol,c->tcp.version,source,ntohs(c->tcp.source),dest,ntohs(c->tcp.dest),c->tcp.sent/1024,c->tcp.received/1024);
						//printf("%s",log);
}
			...
			for (int i = 0; i < ready; i++)
			{
				...
				if (ev[i].data.fd == args->ctx->pipefds[0])
				{
					...
					else
					{
						...
						{
							printf("Total data usage Send:%ld Received:%ld\n",args->send,args->rcvd);
						}
									...
									if(c->tcp.client_state == BLOCKED)
									{
										...
										if(n>0)
										{
											...
											const char *blockmsg="HTTP/1.1 200 OK\r\nServer: Proxy\r\nContent-Type: html\r\nConnection: keep-alive\r\n\r\n<html>"
											 "<h1><center>Domain is blocked by this proxy server</center></h1></html>";
